...
from JokrBackend.models import User, Block
...
def Block(requestData):
    ...
    try:
        ...
        block = Block.objects.filter(blockerUser=clientUser, blockedUser=blockedUser)
        if block:
            DataCollector.logURL(TAG, { 
        ...
        with transaction.atomic():
            Block.objects.create(blockerUser=clientUser, blockedUser=blockedUser)
